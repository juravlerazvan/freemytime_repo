...
class Auth extends CI_Controller {
    ...
    function captcha_check($code) {
        ...
        if ($this->dx_auth->is_captcha_expired()) {
            ...
            $this->form_validation->set_message('captcha_check', '
                <br/><br/>
    ...
    function login() {
        if (!$this->dx_auth->is_logged_in()) {
            ...
            $data['role_id'] = $this->dx_auth->get_role_id();
            $data['role_name'] = $this->dx_auth->get_role_name();
            $data['is_admin'] = $this->dx_auth->is_admin();
            $this->load->view('template', $data);
        }
    }
        ...
        if (!$this->dx_auth->is_logged_in() AND $this->dx_auth->allow_registration) {
            ...
            if ($val->run() AND $this->dx_auth->register($val->set_value('username'), $val->set_value('password'), $val->set_value('email'))) {
                ...
                if ($this->dx_auth->email_activation) {
                    ...
                    $data['auth_message'] = '
                        <br/><br/>
                        You have successfully registered. ' . anchor(site_url($this->dx_auth->login_uri)
                                    . '<br/><br/>', 'Login');
                }

                // Load registration success page
                $data['view_name'] = $this->dx_auth->register_success_view;
                ...
                $data['view_name'] = $this->dx_auth->register_view;

                $this->load->view('template', $data);
            }
        } elseif (!$this->dx_auth->allow_registration) {
            $data['auth_message'] = 'Registration has been disabled.';
            $data['view_name'] = $this->dx_auth->register_disabled_view;
            $this->load->view('template', $data);
        } else {
    ...
    function register_recaptcha() {
        if (!$this->dx_auth->is_logged_in() AND $this->dx_auth->allow_registration) {
            ...
            if ($val->run() AND $this->dx_auth->register($val->set_value('username'), $val->set_value('password'), $val->set_value('email'))) {
                ...
                // Load registration page
                $data['view_name'] = $this->dx_auth->register_recaptcha_form;
                $this->load->view('template', $data);
            }
        } elseif (!$this->dx_auth->allow_registration) {
            $data['auth_message'] = '
            ...
            $data['auth_message'] = '
                <br/><br/>
                You have to logout first, before registering.
                <br/><br/>';
            $data['view_name'] = $this->dx_auth->logged_in_view;
            $this->load->view('template', $data);
        }
    ...
    function activate() {
        ...
        if ($this->dx_auth->activate($username, $key)) {
            $data['auth_message'] = '
                <br/><br/>
                Your account have been successfully activated. ' . anchor(site_url($this->dx_auth->login_uri) . '
                    <br/><br/>', 'Login');

            $data['view_name'] = $this->dx_auth->activate_success_view;
            $this->load->view('template', $data);
        } else {
            $data['auth_message'] = '
                <br/><br/>
                The activation code you entered was incorrect. Please check your email again.
                <br/><br/>';
            $data['view_name'] = $this->dx_auth->activate_failed_view;
            $this->load->view('template', $data);
        }
    }
    ...
    function reset_password() {
        ...
        if ($this->dx_auth->reset_password($username, $key)) {
            $data['auth_message'] = '
                <br/><br/>
                You have successfully reset you password, ' . anchor(site_url($this->dx_auth->login_uri)
                            . '<br/><br/>', 'Login');
            ...
            $data['auth_message'] = '
                <br/><br/>
                Reset failed. Your username and key are incorrect. Please check your email again and follow the instructions.
                <br/><br/>';
    ...
    function cancel_account() {
        ...
        if ($this->dx_auth->is_logged_in()) {
            ...
            if ($val->run() AND $this->dx_auth->cancel_account($val->set_value('password'))) {
                ...
                $data['view_name'] = $this->dx_auth->cancel_account_view;
                $this->load->view('template', $data);
            }
        } else {
            // Redirect to login page
